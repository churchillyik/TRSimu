<form method="post" action="nachrichten.php" name="msg">
	...
	<tr>
		...
		<th colspan="2" class="buttons">
			...
			if ($session->plus)
			{
				echo "<input name=\"archive\" value=\"存档\" type=\"image\" id=\"btn_archiv\" class=\"dynamic_img\" src=\"img/x.gif\" alt=\"存档\" />"; 
			}
			?>
	...
	<tbody>
		...
		for ($i = 1 + $s; $i <= 10 + $s; $i++)
		{
			if (count($message->inbox) >= $i)
			{
				...
				if ($message->inbox[$i - 1]['owner'] <= 1)
				{
					echo "</td><td class=\"send\"><a><u>"
						.$database->getUserField($message->inbox[$i - 1]['owner'], 'username', 0)."</u></a></td>
						<td class=\"dat\">".$date[0]." ".$date[1]."</td></tr>";
				}
				else
				{
					echo "</td><td class=\"send\"><a href=\"spieler.php?uid=".$message->inbox[$i - 1]['owner']."\">"
						.$database->getUserField($message->inbox[$i - 1]['owner'], 'username', 0)."</a></td>
						<td class=\"dat\">".$date[0]." ".$date[1]."</td></tr>";
				}
			}
			$name++;
		...
		{
			echo "<td colspan=\"4\" class=\"none\">没有收到任何讯息。</td></tr>";
		}
		?>
	</tbody>
	</table>
