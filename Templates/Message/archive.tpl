<form method="post" action="nachrichten.php" name="msg" >
	...
	<tr>
		<th colspan="2">主题</th>
		<th>发件人</th>
		<th class="sent"><a href="nachrichten.php?s=0&amp;t=3&amp;o=1">发送时间</a></th>
	</tr>
	</thead>
	
	<tfoot>
	<tr>
		<th><input class="check" type="checkbox" id="s10" name="s10" onclick="Allmsg(this.form);" /></th>
		<th colspan="2" class="buttons">
			<input name="delmsg" value="delete" type="image" id="btn_delete" class="dynamic_img" src="img/x.gif" alt="delete" />
			<input name="start" type="image" value="Back" alt="Back" id="btn_back" class="dynamic_img" src="img/x.gif" />
		</th>
		<th class="navi">
			<?php 
	...
	for ($i = 1 + $s; $i <= 10 + $s; $i++)
	{
		if (count($message->archived) >= $i)
		{
			...
			echo "</td><td class=\"send\"><a href=\"spieler.php?uid=".$message->archived[$i-1]['owner']."\"><u>"
				.$database->getUserField($message->archived[$i-1]['owner'],'username',0)."</u></a></td>
	...
	{
		echo "<td colspan=\"4\" class=\"none\">存档中还没有讯息。</td></tr>";
	}
	?>
	</tbody>
	</table>
