...
if (isset($_GET['send']) && isset($_GET['from']))
{
	...
	if ($_GET['from'] + $max_per_pass <= $users_count)
	{
		$plus = $max_per_pass;
	}
	else
	{
		$plus = $users_count - $_GET['from'];
	}
	...
	{
		echo header("Location: admin.php?p=massmessage&send=true&from=", $_GET['from'] + $max_per_pass);
	}
	else
	{
		$done = true;
...
{
?>
<form method="POST" action="admin.php?p=massmessage" name="myform" id="myform">
...
<div name="smilll" id="message_smilies" style="background:none repeat scroll 0 0 #EFEFEF;border:1px solid #71D000;left:20px;margin-top:5px;max-width:660px;padding:5px;position:relative;display: none;">
	...
	<a href="#" onclick="smilie('*u12*')"><img src="img/x.gif" class="uu12" /></a>
	<br />
	...
	<a href="#" onclick="smilie('*u24*')"><img src="img/x.gif" class="uu24" /></a>
	<br />
...
{
?>
<form method="POST" action="admin.php?p=massmessage">
	<table cellspacing="1" cellpadding="2" class="tbg">    
	<tbody>
	<tr>	
		<td class="rbg" colspan="2">
			<?php echo MASS_CONFIRM; ?>
		</td>
	</tr>
	<tr>	
		<td style="text-align: left; width: 200px;">
			<?php echo MASS_REALLY; ?>
		</td>
...
{
?>
	<script>document.location.href='admin.php?p=massmessage&send=true&from=0'</script>
<?php
}
elseif ($Interupt)
{
