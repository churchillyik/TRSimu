...
if (mysql_num_rows($MyGold))
{
	if ($golds['6'] > 2)
	{
		if (mysql_num_rows($MyVilId))
		{
			...
			$done1 = "&nbsp;&nbsp; 该村庄所有的建筑和研究任务已经完成";
			mysql_query("UPDATE ".TB_PREFIX."users set gold = ".($session->gold - 2)." where `username`='".$session->username."'") or die(mysql_error());
			mysql_query("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog) + 1)."', '".$village->wid."', '成功使用了秒建功能')") or die(mysql_error());
		}
		else
		{
			$done1 = "&nbsp;&nbsp; 该村庄没有建筑或研究处于建造或研发之中";
			mysql_query("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog) + 1)."', '".$village->wid."', '使用秒建功能时失败')") or die(mysql_error());
		}
	}
	else
	{
