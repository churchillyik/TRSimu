...
if (mysql_num_rows($MyGold))
{
	if ($golds['6'] > 2)
	{
		if (mysql_num_rows($MyGold))
		{
			...
			mysql_query("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog) + 1)."', '".$village->wid."', '+25% 粮食产量成功')") or die(mysql_error());
		}
		else
		{
			$done1 = "&nbsp;&nbsp; 使用粮食产量提升失败";
			mysql_query("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog) + 1)."', '".$village->wid."', '+25% 粮食产量失败')") or die(mysql_error());
		}
	}
	else
	{
