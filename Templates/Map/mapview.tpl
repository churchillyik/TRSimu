<div id="content" class="map">
	...
	echo "<h1>中心坐标(<span id=\"x\">".$x."</span>|<span id=\"y\">".$y."</span>)</h1>";
	?>
	<div id="map">
		...
		text_k.einwohner = '人口';
		text_k.allianz = '联盟';
		text_k.verlassenes_tal = '无人绿洲';
		text_k.besetztes_tal = '已占领的绿洲';
		text_k.freie_oase = '未占领的绿洲';
		var text_x = {}
		text_x.r1 = '木材';
		text_x.r2 = '粘土';
		text_x.r3 = '铁矿';
		...
		<map id="map_overlay" name="map_overlay">
			...
			{
				echo "
				<area id=\"a_".$row."_".$i."\" shape=\"poly\" coords=\"".$coorarray[$coorindex]."\" title=\"".$maparray[$coorindex]['name']."\" href =\"karte.php?d=".$maparray[$coorindex]['id']."&c=".$generator->getMapCheck($maparray[$coorindex]['id'])."\" />";
				if ($i == 6 && $row <= 5)
				{
					$row += 1;
					$i = -1;
			...
			?>
			<area id="ma_n1" href="karte.php?z=<?php echo $generator->getBaseID($x,$yp1);?>" coords="422,67,25" shape="circle" title="北"/>
			<area id="ma_n2" href="karte.php?z=<?php echo $generator->getBaseID($xp1,$y);?>" coords="427,254,25" shape="circle" title="东"/>
			<area id="ma_n3" href="karte.php?z=<?php echo $generator->getBaseID($x,$ym1);?>" coords="119,255,25" shape="circle" title="南"/>
			<area id="ma_n4" href="karte.php?z=<?php echo $generator->getBaseID($xm1,$y);?>" coords="114,63,25" shape="circle" title="西"/>
		</map>
		...
		<script type="text/javascript">
			...
			for ($h = 0; $h <= 6; $h++)
			{
				if ($yrow != 7)
				{
					...
					.$maparray[$regcount]['oasistype'].",\"d="
					.$maparray[$regcount]['id']."&c="
					.$generator->getMapCheck($maparray[$regcount]['id'])."\",\""
					...
					if ($maparray[$regcount]['occupied'])
					{
						...
						else
						{
							...
							$text.= ",\"\",\""
							.$database->getUserField($oowner,'username',0)."\",\"-\",\""
		...
		{
			//	target="_blank"
			echo "<a id=\"map_makelarge\" href=\"karte2.php?z=$bigmid\"><img class=\"ml\" src=\"img/x.gif\" alt=\"大地图\" title=\"大地图\"/></a>";
		}
		?>
		<img id="map_navibox" src="img/x.gif" usemap="#map_navibox"/>
		<map name="map_navibox">
			<area id="ma_n1p7" href="karte.php?z=<?php echo $generator->getBaseID($x,$yp7) ?>" coords="51,15,73,3,95,15,73,27" shape="poly" title="北"/>
			<area id="ma_n2p7" href="karte.php?z=<?php echo $generator->getBaseID($xm7,$y) ?>" coords="51,41,73,29,95,41,73,53" shape="poly" title="东"/>
			<area id="ma_n3p7" href="karte.php?z=<?php echo $generator->getBaseID($x,$ym7) ?>" coords="4,41,26,29,48,41,26,53" shape="poly" title="南"/>
			<area id="ma_n4p7" href="karte.php?z=<?php echo $generator->getBaseID($xp7,$y) ?>" coords="4,15,26,3,48,15,26,27" shape="poly" title="西"/>
			<!--z = baseid-->
		</map>
		<div id="map_coords">
			<form name="map_coords" method="post" action="karte.php">
				...
				<input type="image" id="btn_ok" class="dynamic_img" value="确定" name="s1" src="img/x.gif" alt="确定" />
			</form>
		</div>
		<table cellpadding="1" cellspacing="1" id="map_infobox" class="default"><thead><tr><th colspan="2">明细</th></tr></thead><tbody><tr><th>玩家</th><td>-</td></tr><tr><th>人口</th><td>-</td></tr><tr><th>联盟</th><td>-</td></tr>
		</tbody>
		</table>
	</div>
</div>