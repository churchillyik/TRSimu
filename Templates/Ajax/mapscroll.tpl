...
for ($h = 0; $h <= 6; $h++)
{
	if ($yrow != 7)
	{
		...
		$image = ($maparray[$regcount]['occupied'] == 1 && $maparray[$regcount]['fieldtype'] > 0)? 
		(($maparray[$regcount]['owner'] == $session->uid)? 
		($maparray[$regcount]['pop'] >= 50? $maparray[$regcount]['pop'] >= 100? $maparray[$regcount]['pop'] >= 200? 'b30': 'b20' :'b10' : 'b00') : 
		(($targetalliance != 0)? 
		(in_array($targetalliance, $friendarray)? 
		($maparray[$regcount]['pop'] >= 50? $maparray[$regcount]['pop'] >= 100? $maparray[$regcount]['pop'] >= 200? 'b31': 'b21' :'b11' : 'b01') : 
		(in_array($targetalliance, $enemyarray)? 
		($maparray[$regcount]['pop'] >= 50? $maparray[$regcount]['pop'] >= 100? $maparray[$regcount]['pop'] >= 200? 'b32': 'b22' :'b12' : 'b02') : 
		(in_array($targetalliance, $neutralarray)? 
		($maparray[$regcount]['pop'] >= 50? $maparray[$regcount]['pop'] >= 100? $maparray[$regcount]['pop'] >= 200? 'b35': 'b25' :'b15' : 'b05') : 
		($targetalliance == $session->alliance? 
		($maparray[$regcount]['pop'] >= 50? $maparray[$regcount]['pop'] >= 100? $maparray[$regcount]['pop'] >= 200? 'b33': 'b23' :'b13' : 'b03') : 
		($maparray[$regcount]['pop'] >= 50? $maparray[$regcount]['pop'] >= 100? $maparray[$regcount]['pop'] >= 200? 'b34': 'b24' :'b14' : 'b04'))))) : 
		($maparray[$regcount]['pop'] >= 50? $maparray[$regcount]['pop'] >= 100? $maparray[$regcount]['pop'] >= 200? 'b34': 'b24' :'b14' : 'b04'))) : 
