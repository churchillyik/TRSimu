...
<thead>
	<tr>
		<th colspan="2">主题</th>
		<th class="sent"><a href="berichte.php?s=0&amp;o=1">收件时间</a></th>
	</tr>
</thead>
<tfoot>
<tr>
	...
	<th class="buttons">
		<input name="del" type="image" id="btn_delete" class="dynamic_img" src="img/x.gif" value="删除" alt="删除" />
		<?php 
		if ($session->plus)
		{ 
			if (isset($_GET['t']) && $_GET['t'] == 5) 
			{
				echo "<input name=\"start\" type=\"image\" value=\"后退\" alt=\"后退\" id=\"btn_back\" class=\"dynamic_img\" src=\"img/x.gif\" />";
			}
			else
			{
				echo "<input name=\"archive\" type=\"image\" value=\"存档\" alt=\"存档\" id=\"btn_archiv\" class=\"dynamic_img\" src=\"img/x.gif\" />"; 
			}
		}
		?>
	</th>
	<th class=navi>
		...
		else if (isset($_GET['s']) && count($message->noticearray) > $_GET['s'])
		{
			...
			{
				echo "<a href=\"?s=".($_GET['s'] - 10)."&o=0\">&laquo;</a>
					<a href=\"?s=".($_GET['s'] + 10)."&o=0\">&raquo;</a>";
			}
...
<tbody>
	...
	for ($i = 1 + $s; $i <= 10 + $s; $i++)
	{
		...
		{
			echo "<tr><td class=\"sel\"><input class=\"check\" type=\"checkbox\" name=\"n".$name."\" value=\""
				.$message->noticearray[$i-1]['id']."\" /></td><td class=\"sub\">";
			$type = (isset($_GET['t']) && $_GET['t'] == 5)? $message->noticearray[$i-1]['archive'] : $message->noticearray[$i-1]['ntype'];
			echo "<img src=\"img/x.gif\" class=\"iReport iReport$type\" alt=\""
				.$noticeClass[$type]."\" title=\"".$noticeClass[$type]."\" />";
	...
	{
		echo "<td colspan=\"3\" class=\"none\">目前没有任何报告。</td></tr>";
	} 
	?>
</tbody>
</table>
