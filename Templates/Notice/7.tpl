...
<tr>
	<td colspan="2" class="report_content">
		...
		<tr>
			<th>信息</th>
			<td colspan="10">
				<img class="unit u<?php echo $dataarray[142]; ?>" src="img/x.gif" alt="Catapult" title="Catapult" />
				<?php echo $dataarray[141]; ?>
			</td>
		</tr>
		</tbody>
		<?php 
		...
		<tr>
			...
			<td colspan="10">
				...
				<div class="carry">
					<img class="car" src="img/x.gif" alt="carry" title="carry" />
		...
		<thead>
		<tr>
			<td class="role">防御方</th>
			<td colspan="10">
				<?php 
				if ($targettribe=='1')
				{
					echo '<a href="spieler.php?uid='
						.$database->getUserField($dataarray[28],"id",0).'">'
						.$database->getUserField($dataarray[28],"username",0)
						.'</a> 来自于村庄 <a href="karte.php?d='.$dataarray[29]
						.'&amp;c='.$generator->getMapCheck($dataarray[29]).'">'
						.stripslashes($dataarray[30]).'</a>'; 
				}
				else
				{
					echo "增援"; 
		...
		<tr>
			...
			<td colspan="10">
				...
				{ 
					...
					.$database->getUserField($dataarray[28],"id",0).'">'
					.$database->getUserField($dataarray[28],"username",0)
					.'</a> 来自于村庄 <a href="karte.php?d='
					.$dataarray[29].'&amp;c='.$generator->getMapCheck($dataarray[29]).'">'
					.stripslashes($dataarray[30]).'</a>'; 
				} 
				else 
				{ 
			...
			<td colspan="10">
				...
				{ 
					echo '<a href="spieler.php?uid='
						.$database->getUserField($dataarray[28],"id",0).'">'
						.$database->getUserField($dataarray[28],"username",0)
						.'</a> from the village <a href="karte.php?d='
						.$dataarray[29].'&amp;c='.$generator->getMapCheck($dataarray[29]).'">'
						.stripslashes($dataarray[30]).'</a>';
				} 
				else 
		...
		<tr>
			...
			<td colspan="10">
				...
				{ 
					echo '<a href="spieler.php?uid='
						.$database->getUserField($dataarray[28],"id",0).'">'
						.$database->getUserField($dataarray[28],"username",0).'</a> from the village <a href="karte.php?d='
						.$dataarray[29].'&amp;c='.$generator->getMapCheck($dataarray[29]).'">'
						.stripslashes($dataarray[30]).'</a>'; 
				} 
				else 
