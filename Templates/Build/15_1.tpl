...
if ($showcheckdata['timetofinish'] <= time())
{
	if (mysql_num_rows($checkifthereisworking))
	{
		if (($golds['f'.$showcheckdata['buildnumber'].'t'] == "10") 
		OR ($golds['f'.$showcheckdata['buildnumber'].'t'] == "11"))
		{
...
{
	echo "<h2>拆除建筑：</h2>
		<p>如果你有哪个建筑不需要的，你可以给你的建筑师发令让他将你不需要的建筑慢慢拆除。</p>";
	if (mysql_num_rows($checkifthereisworking))
	{
		echo "<b>";
		echo "<a href='build.php?id=26&cancel=1'>
			<img src='img/x.gif' class='del' title='取消' alt='取消'></a>";
		echo "<a>拆除 ".$building->procResType($golds['f'.$showcheckdata['buildnumber'].'t'])."
			<span id=timer1>".$generator->getTimeFormat($showcheckdata['timetofinish'] - time())."</span> 小时</a>";
		echo "</b>";
	}
	else
	{
		...
		<input id=\"btn_demolish\" name=\"ok\" class=\"dynamic_img\" type=\"image\" src=\"img/x.gif\" alt=\"拆除\"/>
		</form>";
	}
}
?>