...
if($village->resarray['f'.$id] >= 10)
{
	...
	if (mysql_num_rows($checkifthereisworking)) {
		...
		echo "<a>Demolition ".$building->procResType($golds['f'.$showcheckdata['buildnumber'].'t'])." <span id=timer1>".$generator->getTimeFormat($showcheckdata['timetofinish']-time())."</span> hour left</a>";
		echo "</b>";
	}
	else
	{
