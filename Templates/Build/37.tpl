...
if (isset($_GET['add']))
{
	if ($_GET['add'] == reset)
	{
		if (round(($heroinfo1['attackpower'] + $heroinfo1['defpower'] 
			+ $heroinfo1['attackbonus'] + $heroinfo1['defbonus'] + $heroinfo1['regspeed']) / 5) <= $resetlevelreq )
		{
...
<div id="build" class="gid37">
	<a href="#" onClick="return Popup(37,4, 'gid');" class="build_logo">
	...
	<tbody>
		...
		<tr>
			...
			<th width='25%'><a href='build.php?id=<?php echo "$id"; ?>&train=1'>训练</a></th>
		</tr>
		<tr>
			<th width='75%'><img class="unit u2" src="img/x.gif"><?php echo $typercheck['2']; ?></th>
			<th width='25%'><a href='build.php?id=<?php echo "$id"; ?>&train=2'>训练</a></th>
		</tr>
		<tr>
			<th width='75%'><img class="unit u3" src="img/x.gif"><?php echo $typercheck['3']; ?></th>
			<th width='25%'><a href='build.php?id=<?php echo "$id"; ?>&train=3'>训练</a></th>
		</tr>
		<tr>
			<th width='75%'><img class="unit u5" src="img/x.gif"><?php echo $typercheck['5']; ?></th>
			<th width='25%'><a href='build.php?id=<?php echo "$id"; ?>&train=4'>训练</a></th>
		</tr>
		<tr>
			<th width='75%'><img class="unit u6" src="img/x.gif"><?php echo $typercheck['6']; ?></th>
			<th width='25%'><a href='build.php?id=<?php echo "$id"; ?>&train=5'>训练</a></th>
		</tr>
		<?php
		}
		if ($session->tribe == 2)
	...
	<tr>
		<th colspan="5">
			<a href="build.php?id=<?php echo "$id"; ?>&rename"><?php echo $heroinfo['heroname']; ?></a>
			<span class="info">
				等级 <?php echo $level; ?> （<img class="unit u<?php echo $heroinfo['type']; ?>" src="img/x.gif" alt="<?php echo $heronames; ?>" title="<?php echo $heronames; ?>" /><?php echo $heronames; ?>）
			</span>
		</th>
	</tr>
	</thead>
	...
	<tr>
		...
		<td class="xp">
			<img class="bar" src="img/x.gif" style="width:<?php echo $heroinfo['attackpower']; ?>%;" alt="<?php echo $heroattackpower; ?>" title="<?php echo $heroattackpower; ?>" />
		</td>
		<?php
		if ($heropoint >= 1)
		{
	...
	<tr>
		...
		<td class="xp">
			<img class="bar" src="img/x.gif" style="width:<?php echo $heroinfo['defpower']; ?>%;" alt='<?php echo  $herodefpower1."/".$herodefpower2; ?>' title='<?php echo  $herodefpower1."/".$herodefpower2; ?>' />
		</td>
		<?php
		if ($heropoint >= 1)
		{
	...
	<tr>
		...
		<td class="xp">
			<img class="bar" src="img/x.gif" style="width:<?php echo $heroinfo['attackbonus']; ?>%;" alt="<?php echo ($heroinfo['attackbonus'] * 0.2); ?>%" title="<?php echo ($heroinfo['attackbonus'] * 0.2); ?>%" />
		</td>
		<?php
		if ($heropoint >= 1)
		{
	...
	<tr>
		...
		<td class="xp">
			<img class="bar" src="img/x.gif" style="width:<?php echo $heroinfo['defbonus']; ?>%;" alt="<?php echo ($heroinfo['defbonus'] * 0.2); ?>%" title="<?php echo ($heroinfo['defbonus'] * 0.2); ?>%" />
		</td>
		<?php
		if ($heropoint >= 1)
		{
	...
	<tr>
		<th>体力恢复：</th>
		<td class="val"><?php echo ($heroinfo['regspeed']*1); ?> / 天</td>
		<td class="xp">
			<img class="bar" src="img/x.gif" style="width:<?php echo $heroinfo['regspeed']; ?>%;" alt="<?php echo ($heroinfo['regspeed']*1); ?> / 天" title="<?php echo ($heroinfo['regspeed']*1); ?> / 天" />
		</td>
		<?php
		if ($heropoint >= 1)
		{
	...
	<tr>
		<th title="hero is health">经验值：</th>
		<td class="val"><?php echo $expernce; ?>%</td>
		<td class="xp">
			<img class="bar" src="img/x.gif" style="width:<?php echo $expernce; ?>%;" alt="<?php echo $expernce; ?>%" title="<?php echo $expernce; ?>%" />
		</td>
		<td class="up"></td>
		<td class="rem">
			<?php
	...
	{
		echo "<br>";
		echo "如果你的英雄在".$resetlevelreq."级以下，你可以点击<a href='build.php?id=".$id."&add=reset'>这里</a>重置英雄的能力点数。";
	}
	?>
	
	<p>你的英雄的体力值还剩下 <b><?php echo $heroinfo['healthofhero']; ?></b>%<br/></p>
