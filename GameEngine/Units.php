...
class Units
{
	...
	private function loadUnits($post)
	{
		...
		if (isset($post['x']) && isset($post['y']) && $post['x'] != "" && $post['y'] != "")
		{
			...
			for ($i = 1; $i < 11; $i++)
			{
				if (isset($post['t'.$i]))
				{
					...
					{
						$form->addError("error", "发兵数超过了你拥有的兵数");
						break;
					}
					
					if ($post['t'.$i] < 0)
					{
						$form->addError("error", "发兵数不能为负数");
						break;
					}
				}												
			}
		...
		if ($userAccess == '0')
		{
			$form->addError("error", "该玩家已被封号，无法攻击。");
		}
	...
	private function sendTroops($post)
	{
		...
		else
		{
			...
			if ($data2['protect'] < time())
			{
				...
				$reference = $database->addAttack($village->wid
	...
	private function sendTroopsBack($post)
	{
		...
		{
			$form->addError("error", "你无法改变别人的军队");
			if ($form->returnErrors() > 0)
			{
				$_SESSION['errorarray'] = $form->getErrors();
				$_SESSION['valuearray'] = $_POST;
