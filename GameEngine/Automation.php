...
{	
	...
	private function sendunitsComplete()
	{
		...
		foreach($dataarray as $data)
		{
			...
			for ($i = 19; $i < 40; $i++)
			{
				if ($database->getFieldLevel($data['to'], "".$i."t") == '25'
					or $database->getFieldLevel($data['to'], "".$i."t") == '26')
				{
			...
			if ($type == '3')
			{
				if ($catp != '0')
				{
					if (isset($empty))
					{
						$info_cat = ",".$catp_pic.",村庄已被完全摧毁。";
					}
					elseif ($battlepart[4] > $battlepart[3])
					{	
						$info_cat = "".$catp_pic.",".$this->procResType($tbgid)." 已被摧毁。";
					...
					{
						$info_cat = "".$catp_pic.",".$this->procResType($tbgid)." 没有受到伤害。";
					}
					else
					{
						$demolish = $battlepart[4] / $battlepart[3];
						$totallvl = round(sqrt(pow(($tblevel + 0.5), 2) - $battlepart[4] * 8));
						$info_cat = "".$catp_pic.",".$this->procResType($tbgid)." 遭到攻击从等级 <b>".$tblevel."</b> 下降到等级 <b>".$totallvl."</b>.";
			...
			if ($data['t9'] - $dead9) > 0)
			{
				...
				if (count($varray) != '1' and $to['capital'] != '1')
				{
					for ($i = 18; $i < 39; $i++)
					{
						...
						{
							$nochiefing = '1';
							$info_chief = "".$chief_pic.",皇宫或行宫还未被摧毁。";
						}
					}
					if (!isset($nochiefing))
					{
						...
						{
							$info_chief = "".$chief_pic.",村庄的忠诚度从 <b>".$toF['loyalty']."</b> 下降到 <b>".($toF['loyalty']-$rand)."</b>.";
							$database->setVillageField($data['to'],loyalty,($toF['loyalty']-$rand));
						}
						else
						{
							$info_chief = "".$chief_pic.",你已经取得该村庄的控制权。";
				...
				{
					$info_chief = "".$chief_pic.",你还不能占领该村庄。";
				}
			}
